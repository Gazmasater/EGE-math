...
Сложим равенства Pᵢ=qDᵢ₋₁−Dᵢ по всем годам. Все промежуточные остатки учитываются один раз после начисления и один раз после платежа, поэтому сумма выплат равна первоначальному долгу плюс проценты на все остатки, существовавшие перед очередным начислением. Именно эта модель объясняет суммы долгов и формулы ниже; она не подменяет долг после платежа долгом до начисления процентов.

Если по условию ежегодные платежи одинаковы и равны P, то последовательное применение перехода даёт Dₙ=qⁿD₀−P(1+q+…+qⁿ⁻¹). При полном погашении Dₙ=0. Если платежи или остатки заданы иначе, тот же переход Pᵢ=qDᵢ₋₁−Dᵢ применяем по годам по отдельности.
