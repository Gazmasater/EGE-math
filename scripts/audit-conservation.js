...
const { records } = require('./data/conservation-solutions');
const { diagrams } = require('./data/conservation-diagrams');
const sources = require('./data/conservation-sources.json');
const root = path.resolve(__dirname, '..');
const ids = Object.keys(records).sort();
const expected = require('./data/conservation-answers.json');
function verifyPhysics() { require('./data/conservation-checks').verifyPhysics(); }
function verifyContent() {
  assert.equal(ids.length,36,'Число задач текущего раздела КЭС 1.4');
  assert.deepEqual(ids,Object.keys(expected).sort(),'Полнота 36 решений');
  ...
  for(const id of ids) {
    ...
    assert.ok(diagrams[id].includes(`data-conservation-diagram="${id}"`),`${id}: схема соответствует задаче`);
