...
const { records } = require('./data/dynamics-solutions');
const { diagrams } = require('./data/dynamics-diagrams');
const sources = require('./data/dynamics-sources.json');
const root = path.resolve(__dirname, '..');
const ids = Object.keys(records).sort();
const expected = require('./data/dynamics-answers.json');
const close=(a,b,message)=>assert.ok(Math.abs(a-b)<1e-9,`${message}: ${a} ≠ ${b}`);
function verifyPhysics() { require('./data/dynamics-checks').verifyPhysics(); }
function verifyContent() {
  assert.deepEqual(ids,Object.keys(expected).sort(),'Полнота 39 решений');
  ...
  for(const id of ids) {
    ...
    assert.ok(diagrams[id].includes(`data-dynamics-diagram="${id}"`),`${id}: схема соответствует задаче`);
