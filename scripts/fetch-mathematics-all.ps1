...
$project = 'AC437B34557F88EA4115D2F374B0A07B'
...
for ($page = 0; $page -lt 100; $page++) {
    $target = [IO.Path]::GetFullPath((Join-Path $projectDir "mathematics-$($page + 1).raw.html"))
    ...
    if ($page -eq 0 -and $count -eq 0) {
        Remove-Item -LiteralPath $temporary -Force -ErrorAction SilentlyContinue
        throw 'FIPI returned no profile-mathematics tasks'
    }
...
Get-ChildItem -LiteralPath $projectDir -Filter 'mathematics-*.raw.html' -File | ForEach-Object {
...
Write-Output "mathematics_tasks=$($taskIds.Count) assets=$($assetPaths.Count)"
